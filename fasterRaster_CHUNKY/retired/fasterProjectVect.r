...
#'
#' @param template Either \code{NULL} (default), \emph{or} a \code{SpatRaster}, \code{SpatVector}, or object of class \code{sf} to serve as a template for projecting. If there is an existing \code{GRASS} session (started by another \pkg{fasterRaster} function or through the \code{\link{startFaster}} function), then this can be \code{NULL}, and the vector in \code{vect} will be projected to the one used by the current \code{GRASS} session.  However, if a \code{GRASS} session has not yet been started, or a different projection is desired, then this argument must be non-\code{NULL}.
...
fasterProjectVect <- function(
	...
	fromGrass <- startFaster(rast=NULL, vect=vect, location='fromVect', inRastName=NULL, inVectName=inVectName, replace=replace, grassDir=grassDir)
	...
	if (buildTopo) flags <- c(thisFlags, 'b')
